...
```{r plot routes with home and office, include=FALSE}
...
#### 4.2.1.2 Vehicle ID 35

```{r route 28, eval=FALSE, include=FALSE}
#15 has 2 matches for 1900hrs
car_ID <- 35

card_i <- print_map <- save_emp_routes_i(car_ID, "2014-01-13")

tmap_mode("view")

card_i

card_POIs <- records_POI %>%
    filter(id == car_ID) %>%
    filter(datestamp == "2014-01-13") %>%
    select(-c(visitcount, lat111, long111)) %>%
    group_by(lat, long) %>%
...
```{r plot moving chart, include=FALSE}
...
```{r anomalies in movement, echo=FALSE}
